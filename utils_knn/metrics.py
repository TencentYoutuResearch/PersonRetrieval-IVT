...
def fowlkes_mallows_score(gt_labels, pred_labels, sparse=True):
    ...
    tk = np.dot(c.data, c.data) - n_samples         # 34078
    pk = np.sum(np.asarray(c.sum(axis=0)).ravel()**2) - n_samples       # 38514
    qk = np.sum(np.asarray(c.sum(axis=1)).ravel()**2) - n_samples       # 42210

    avg_pre = tk / pk       # 0.8848
    avg_rec = tk / qk       # 0.8073
    fscore = _compute_fscore(avg_pre, avg_rec)      # 0.84430

    return avg_pre, avg_rec, fscore       # 0.8848, 0.8073, 0.84430
...
def bcubed(gt_labels, pred_labels):
    ...
    fscore = _compute_fscore(avg_pre, avg_rec)  # ave_pre: 0.9759, ave_rec: 0.7356, fscore: 0.8389
