...
from utils_knn import (load_data, dump_data, mkdir_if_no_exists, Timer)
...
def knns2ordered_nbrs(knns, sort=True):
    if isinstance(knns, list):
        knns = np.array(knns)
    nbrs = knns[:, 0, :].astype(np.int32)           # [25752, 5]
    dists = knns[:, 1, :]          # [25752, 5]
    if sort:
        # sort dists from low to high
        nb_idx = np.argsort(dists, axis=1)          # [25752, 5]
        idxs = np.arange(nb_idx.shape[0]).reshape(-1, 1)          # [25752, 1]
        dists = dists[idxs, nb_idx]                 # [25752, 5]
        nbrs = nbrs[idxs, nb_idx]                   # [25752, 5]
    return dists, nbrs          # [25752, 5], [25752, 5]
...
def fast_knns2spmat(knns, k, th_sim=0.7, use_sim=False, fill_value=None):
    ...
    if len(knns.shape) == 2:
        ...
        knns = ndarr
    nbrs = knns[:, 0, :]            # [25752, 5], min=0, max=25751
    dists = knns[:, 1, :]           # [25752, 5], min=-2.3e-7, max=0.449
    assert -eps <= dists.min() <= dists.max(
    ) <= 1 + eps, "min: {}, max: {}".format(dists.min(), dists.max())
    if use_sim:
        sims = 1. - dists           # [25752, 5]
    else:
        sims = dists
    ...
    row = row[idxs]                 # [103008,]
    col = col[idxs]                 # [103008,]
    data = sims[row, col]           # [103008,]
    col = nbrs[row, col]            # [103008,], convert to absolute column
    assert len(row) == len(col) == len(data)
    spmat = csr_matrix((data, (row, col)), shape=(n, n))
    return spmat
    ...
    def __init__(self,
        ...
        with Timer('[faiss] query topk {}'.format(k), verbose):
            ...
            if os.path.exists(knn_ofn):
                ...
                sims, nbrs = index.search(feats, k=k)           # [26960, 5], [26960, 5]
                self.knns = [(np.array(nbr, dtype=np.int32),
