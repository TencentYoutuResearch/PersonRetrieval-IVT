...
def read_probs(path, inst_num, feat_dim, dtype=np.float32, verbose=False):
    ...
    probs = np.fromfile(path, dtype=dtype, count=count)     # [13803520,]
    if feat_dim > 1:
        probs = probs.reshape(inst_num, feat_dim)           # [25752, 256]
    if verbose:
...
def read_meta(fn_meta, start_pos=0, verbose=True):
    ...
    inst_num = len(idx2lb)          # 25752
    cls_num = len(lb2idxs)          # 3991
    if verbose:
        print('[{}] #cls: {}, #inst: {}'.format(fn_meta, cls_num, inst_num))        # #cls: 3991, #inst: 25752
...
def write_meta(ofn, idx2lb, inst_num=None):
    ...
    cls_num = len(set(idx2lb.values()))         # 5609

    idx2newlb = {}
    current_lb = 0
    discard_lb = 0
    map2newlb = {}
    for idx in range(inst_num):         # 26960
        if idx in idx2lb:
            lb = idx2lb[idx]
            if lb in map2newlb:
                newlb = map2newlb[lb]
