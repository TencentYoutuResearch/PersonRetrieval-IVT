...
class BertLayer(nn.Module):
    ...
    def feed_forward_chunk2(self, attention_output):
...
class BertOnlyMLMHead(nn.Module):
    ...
    def forward(self, sequence_output):  # [8, 13, 768]
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0]  # [8, 13, 768]
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]  # [8, 13, 768]
        if data_mode == 'cloth':
            prediction_scores = self.cls(sequence_output)   # [8, 13, 30522]
        else:
            prediction_scores = self.cls2(sequence_output)  # [8, 13, 30522]

        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = padding token
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))  # 5.5074
