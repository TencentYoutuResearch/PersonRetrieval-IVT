...
from model.backbones.transformer import build_vis_transformer, build_transformer
from model.backbones.position_encoding import build_position_encoding
...
class VLFusion(nn.Module):
    def __init__(self, transformer, pos):
        ...
        self.pos = pos
        hidden_dim = transformer.d_model
        self.pr = nn.Embedding(1, hidden_dim)           # 1->256
    ...
    def forward(self, fv, fl, mask):     # [8, 256, 20, 20], [8, 40, 768]
        """ The forward expects a NestedTensor, which consists of:
               - samples.tensor: batched images, of shape [batch_size x 3 x H x W]
               - samples.mask: a binary mask of shape [batch_size x H x W], containing 1 on padded pixels
        """
        ...
        # pv = self.v_proj(fv)  # [bs,400,256]
        # pl = self.l_proj(fl)  # [bs, 40, 256]
        pv = fv.permute(0, 2, 1)  # [bs,256,400]
        pl = fl.permute(0, 2, 1)  # [bs,256,40]
        ...
        x0 = torch.cat((pv, pl), dim=2)             # [bs, 256, 440]
        x0 = torch.cat((x0, pr), dim=2)             # [bs, 256, 441]

        pos = self.pos(x0).to(x0.dtype)             # [bs, 441, 256]
        reg_msk = torch.ones([bs, 1]).cuda()        # [8, 441]
        mask = 1 - torch.cat([mask, reg_msk], dim=-1)
