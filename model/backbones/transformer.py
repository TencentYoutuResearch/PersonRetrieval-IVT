...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        # tgt是query embedding，shape是(num_queries, b, hidden_dim)[num_queries, 64, 256]
        # query_pos是对应tgt的位置编码，shape和tgt一致(num_queries, b, hidden_dim)[num_queries, 64, 256]
        # memory是Encoder的输出，shape是(w, b, hidden_dim)[441, 64, 256]
        # memory_key_padding_mask对应Encoder的src_key_padding_mask
        # 也是EncoderLayer的key_padding_mask，shape是(b, w)[64, 441]
        # pos对应输出到Encoder的位置编码，这里代表memory的位置编码，shape和memory一致
        output = tgt
...
def build_transformer(args):
    return Transformer(
        d_model=args.TEXT.HIDDEN_DIM,
        dropout=args.TEXT.DROPOUT,
        nhead=args.TEXT.NHEADS,
        dim_feedforward=args.TEXT.DIM_FEEDFORWARD,
        num_encoder_layers=args.TEXT.ENC_LAYERS,
        num_decoder_layers=args.TEXT.DEC_LAYERS,
        normalize_before=args.TEXT.PRE_NORM,
