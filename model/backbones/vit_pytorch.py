...
class Attention(nn.Module):
    ...
    def forward(self, x):     # [64, 129, 768]
        ...
        attn = attn.softmax(dim=-1)         # [64, 12, 129, 129]
        attn = self.attn_drop(attn)         # [64, 12, 129, 129]

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)     # [64, 129, 768]
        x = self.proj(x)                    # [64, 129, 768]
        x = self.proj_drop(x)               # [64, 129, 768]
        return x
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))       # [1, 129, 768]
    ...
    def forward_features(self, x, camera_id, view_id):      # [64, 3, 256, 128]
        ...
        if self.cam_num > 0 and self.view_num > 0:
            ...
            x = x + self.pos_embed + self.sie_xishu * self.sie_embed[camera_id]     # [64, 129, 768]
        ...
        if self.local_feature:
            ...
            x = self.norm(x)             # [64, 129, 768]

            return x[:, 0]               # [64, 768]

    def forward(self, x, cam_label=None, view_label=None):      # [64, 3, 256, 128]
        x = self.forward_features(x, cam_label, view_label)     # [64, 129, 768]
        return x
